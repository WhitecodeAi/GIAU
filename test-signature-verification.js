...
async function testFormGI3A() {
  try {
    ...
    const response = await fetch(
      "http://localhost:8080/api/registrations/export-product-gi3a",
      {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
      ...
      const hasImageTag =
        htmlContent.includes("<img") && htmlContent.includes('alt="Signature"');
      ...
      const signatureAreaMatch = htmlContent.match(
        /<div class="signature-area">[\s\S]*?<\/div>/,
      );
...
async function testStatement() {
  try {
    ...
    const response = await fetch(
      "http://localhost:8080/api/registrations/export-product-statement",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      ...
      const hasSignatureSrc = htmlContent.includes(
